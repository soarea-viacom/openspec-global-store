...
- Autonomous-orchestration config (`orchestration.concurrency`, `gate_quick`, `gate_full`) goes in the **store's** `openspec/config.yaml` (`~/openspec-stores/<slug>/openspec/config.yaml`) — never in the project. This is the single location the engine reads.
